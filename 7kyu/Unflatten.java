...
Pseudocode:
Create an ArrayList
Loop through the array, for each n, 
  if n is greater than 2
    take a copy of the array from current index to current index + curr index value
    push values to arrayList
    increment i by index value
  else push value to ArrayList
...
public class Unflatten {
  public static Object[] unflatten(int[] flatArray) {
    ...
    int arrSize = flatArray.length;
    for (int i = 0; i < arrSize; i++) {
      if (flatArray[i] > 2) {
        int end = Math.min(i + flatArray[i], arrSize);
        arrList.add(Arrays.copyOfRange(flatArray, i, end));
        i = end - 1;
      } else
        arrList.add(flatArray[i]);
    }
