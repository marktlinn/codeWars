...
\\\\pseudocode////
Loop over the arr, for each correct answer increment the value by the "right" amount, for each "omit" add the amount so that it subtracts if necessary, for each "wrong" answer decrement the count.
return the count
*/

function scoreTest(str, right, omit, wrong) {
  return str.reduce((a, b) => {
    if (b === 0) return (a += 1 * right);
    if (b === 1) return (a += omit);
    if (b === 2) return (a -= 1 * wrong);
  }, 0);
}
