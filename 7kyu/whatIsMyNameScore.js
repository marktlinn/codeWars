...
\\\\Pseudocode/plan////
cretea a var to upperCase, trim and split name into individual letters
declare ttl var  = 0
    for each letter loop through the alpha object to match the letter,
        if the letter matches any key add the value from that key to the ttl var
    finally create an object to hold a {name: ttl} pairing and return
*/
alpha={'ABCDE':1,'FGHIJ':2,'KLMNO':3,'PQRST':4,'UVWXY':5};

function nameScore(name){
    let ttl = 0;
    const splitName = name.toUpperCase().trim().split('');
    let index = 0;
    while (index<splitName.length){
        for(let key in alpha){
            if(key.match(splitName[index])){
                ttl+= alpha[key];
            }
        }
        index++;
...
console.log(nameScore('Stone Ellery Bernardo Elise Green'))