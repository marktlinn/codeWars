...
/*
Link: https://www.codewars.com/kata/62d34faad32b8c002a17d6d9/train/java
...
public class PageReplacement {
  private static int resetPointer(int lengthOfArr, int pointer) {
  ...
  public static int[] fifo(int n, int[] referenceList) {
    ...
    for (int i : referenceList) {
      if (!Arrays.stream(queue).anyMatch(num -> num == i)) {
        queue[pointer] = i;
        pointer++;
      }
      pointer = resetPointer(n, pointer);
    }
    return queue;
  }
}