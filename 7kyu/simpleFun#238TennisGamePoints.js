...
\\\\Pseudocode////
create a helper function that runs a switch statement that converts the tennis scores to scores between 1-4
in the main function split the string at hte hyphen, and reduce the contents
  pass each elem through the helper function to get their score and then add them to the total
  if current element === "all", add the ttl to the ttl
...
function tennisGamePoints(score) {
  const tennisScoreArray = score.split("-");
  return tennisScoreArray.reduce((ttl, curr) => {
    const score = pointConverter(curr);
    if (score === "all") {
      ttl += ttl;
    } else {
      ttl += score;
