...
filter through the results, 
  times the floor by six to get the last index to check.
  subtract 6 from the last index to check to get the first door on the floor to check
    if array's index is >= start && < last index retrun elem.
*/

function roomMates(rooms, floor) {
  return rooms.filter((elem, index) => {
    const lastDoor = floor * 6;
    start = lastDoor - 6;
    if (index >= start && index < lastDoor) {
      return elem;
    }
  });
}
...
console.log(
  roomMates(["foo", "bar", "foobar", "barfoo", "foofoo", "barbar"], 1)
);
console.log(
  roomMates(["foo", "bar", "foobar", "barfoo", "foofoo", "barbar"], 2)
);
console.log(
  roomMates(
